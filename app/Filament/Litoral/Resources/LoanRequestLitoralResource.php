...
use App\Filament\Litoral\Resources\LoanRequestLitoralResource\Pages;
...
class LoanRequestLitoralResource extends Resource
{
    protected static ?string $model = LoanRequestLitoral::class;
                        ...
                        Forms\Components\Grid::make(3)
                            ->schema([
                                ...
                                Forms\Components\Select::make('payment_frequency')
                                    ...
                                    ->options(LoanRequestLitoral::PAYMENT_FREQUENCIES)
                                    ->required(),

                                Forms\Components\Select::make('loan_reason')
                                    ->label('Motivo del Préstamo')
                                    ->options(LoanRequestLitoral::LOAN_REASONS)
                                    ->required(),
                        ...
                        Forms\Components\Grid::make(1)
                            ->schema([
                                ...
                                Forms\Components\Hidden::make('loan_number')
                                    ->default(fn() => LoanRequestLitoral::generateLoanNumber()),
                            ...
                            ->schema([
                                ...
                                Infolists\Components\Group::make([
                                    ...
                                    Infolists\Components\TextEntry::make('payment_frequency')
                                        ->label('Frecuencia de Pago')
                                        ->formatStateUsing(fn(string $state): string => LoanRequestLitoral::PAYMENT_FREQUENCIES[$state] ?? $state),
                                    Infolists\Components\TextEntry::make('loan_reason')
                                        ->label('Motivo del Préstamo')
                                        ->formatStateUsing(fn(string $state): string => LoanRequestLitoral::LOAN_REASONS[$state] ?? $state),
                                    ...
                                    Infolists\Components\TextEntry::make('status')
                                        ->label('Estado')
                                        ->badge()
                                        ->formatStateUsing(fn(string $state): string => LoanRequestLitoral::STATUSES[$state] ?? $state)
        ...
        return $table
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('status')
                    ->label('Estado')
                    ->options(LoanRequestLitoral::STATUSES),
            ])
            ->actions([
                ViewAction::make()
                    ->modalWidth('4xl'),
                ...
                Action::make('approve')
                    ...
                    ->action(function (LoanRequestLitoral $record) {
                        try {
                            $activeLoan = DB::transaction(function () use ($record) {
                                $record->update(['status' => 'approved']);
                                return ActiveLoanLitoral::createFromRequest($record);
                            });
                            ...
                            return redirect()->to('litoral/loan-request-litorals');
                        } catch (\Exception $e) {
                            Notification::make()
    ...
    {
        return [
            'index' => Pages\ListLoanRequestLitorals::route('/'),
        ];
    }
}
